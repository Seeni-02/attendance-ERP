...
import { getZoneInfo, simulateSMS, formatDate, calculateAttendancePercentage, generateStudentId, calculateZone } from '../utils/helpers';
...
interface AttendanceSheetProps {
  ...
  onMarkBulkAttendance: (records: AttendanceRecord[], updatedStudents: Student[]) => void;
...
const AttendanceSheet: React.FC<AttendanceSheetProps> = ({
  ...
  const handleAddNewStudent = () => {
    if (!newName.trim()) { setAddError('Please enter student name'); return; }
    if (!newRegNumber.trim()) { setAddError('Please enter register number'); return; }
    if (!newPhone.trim() || newPhone.replace(/\D/g, '').length < 10) { setAddError('Please enter a valid 10-digit mobile number'); return; }

    const cleanReg = newRegNumber.trim().toUpperCase();
    const cleanPhone = newPhone.replace(/\D/g, '').slice(-10);

    // Check duplicate register number in this class
    const duplicate = students.find(s => !s.isRemoved && s.Register_Number.toUpperCase() === cleanReg);
    if (duplicate) {
      setAddError(`Register number "${cleanReg}" already exists for ${duplicate.Student_Name}`);
      return;
    }

    const newStudent: Student = {
      Student_ID: generateStudentId(className, students.length),
      ...
      Zone: 'blue-star',
      isRemoved: false,
      Join_Date: currentDate, // New student joins from today
    };

    onAddStudent(newStudent);
    onAddNotification({
      type: 'success',
      message: `✅ Student "${newStudent.Student_Name}" (${cleanReg}) added successfully!`
    });
  ...
  // A student is eligible to mark attendance on currentDate only if their Join_Date <= currentDate
  const isStudentEligibleForDate = (student: Student): boolean => {
    if (!student.Join_Date) return true; // Legacy students with no join date are always eligible
    return student.Join_Date <= currentDate;
  };
  ...
  const hasAbsentOrODToday = todayRecords.some(r => r.Status === 'absent' || r.Status === 'od');
  const todayAbsentOrODCount = todayRecords.filter(r => r.Status === 'absent' || r.Status === 'od').length;

  // Students eligible on this date and not yet marked
  const eligibleUnmarkedStudents = activeStudents.filter(s =>
    isStudentEligibleForDate(s) && !attendanceRecords.some(r => r.Student_ID === s.Student_ID && r.Date === currentDate)
  );

  const canMarkPresentForOthers = hasAbsentOrODToday && eligibleUnmarkedStudents.length > 0;

  // 1. Mark Present for All handler (directly marks all unmarked eligible students as present to save time)
  const handleMarkPresentForAll = async () => {
    if (eligibleUnmarkedStudents.length === 0 || isBulkMarking) return;
    setIsBulkMarking(true);

    const records: AttendanceRecord[] = eligibleUnmarkedStudents.map(s => ({
    ...
    const updatedStudents: Student[] = eligibleUnmarkedStudents.map(s => ({
      ...s,
      Total_Days_Present: s.Total_Days_Present + 1,
      Zone: calculateZone(s.Total_Days_Absent, s.On_Duty_Days),
    }));

    onMarkBulkAttendance(records, updatedStudents);
    onAddNotification({
      type: 'success',
      message: `✅ All ${eligibleUnmarkedStudents.length} student(s) marked as Present for ${formatDate(currentDate)}`
    });
    setIsBulkMarking(false);
  };

  // 2. Mark Present for Others handler (only enabled when teacher marked absent/od for at least 1 student)
  const handleMarkPresentForOthers = async () => {
    if (!canMarkPresentForOthers || isBulkMarking) return;
    setIsBulkMarking(true);
    ...
    onAddNotification({
      type: 'success',
      message: `✅ Marked remaining ${eligibleUnmarkedStudents.length} student(s) as Present for ${formatDate(currentDate)}`
    });
    setIsBulkMarking(false);
  };
    ...
    <div className="bg-gray-100">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all flex items-center gap-1.5 ${isHoliday
          ...
          <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
            <div className="flex items-center gap-2 text-orange-700">
              ...
              <span className="font-semibold">
                {new Date(currentDate).toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'short', year: 'numeric' })} is marked as a Holiday {currentHoliday?.reason ? `(${currentHoliday.reason})` : ''}
              </span>
            </div>
            <button
        ...
        <div className="grid grid-cols-5 gap-3 mb-4">
          {[
            ...
            <button key={item.zone}
              ...
              className={`p-3 rounded-xl transition-all ${filter === item.zone ? `${item.color} text-white shadow-lg` : 'bg-white hover:shadow-md'
                }`}>
      ...
      <div className="max-w-7xl mx-auto px-4 pb-8">
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <div className="flex items-center gap-2">
              {isAuthenticated && (
                <button
                  ...
                  className="px-3.5 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-lg hover:from-blue-700 hover:to-indigo-700 transition-all font-semibold text-sm shadow flex items-center gap-1.5"
                >
              ...
              {!isAuthenticated && classPassword && (
                <button
                  ...
                  className="px-4 py-2 text-sm bg-yellow-100 text-yellow-700 rounded-lg hover:bg-yellow-200 transition-colors font-medium flex items-center gap-1.5"
                >
                  <span>🔐</span> Unlock Edit Mode
                </button>
              )}
            </div>
          </div>
            ...
            <table className="w-full">
              ...
              <tbody>
                {filteredStudents.map((student, index) => {
                  ...
                  const isEligible = isStudentEligibleForDate(student);

                  return (
                    <tr key={student.Student_ID} className={`border-b hover:bg-gray-50 ${todayRecord ? 'bg-gray-50' : ''} ${!isEligible ? 'opacity-60' : ''}`}>
                      ...
                      <td className="px-4 py-3">
                        {/* HOLIDAY: show blank dash */}
                        {isHoliday ? (
                          <div className="flex items-center justify-center">
                            <span className="text-gray-300 text-xl font-bold" title="Holiday — no attendance">—</span>
                          </div>
                        ) : !isEligible ? (
                          /* Student not yet joined on this date */
                          <div className="flex items-center justify-center">
                            <span className="px-2 py-1 bg-gray-100 text-gray-400 rounded-full text-xs" title={`Joins on ${student.Join_Date}`}>Not enrolled</span>
                          </div>
                        ) : todayRecord ? (
                          <div className="flex items-center justify-center">
          ...
          {/* Bulk Attendance Action Bar */}
          {!isHoliday && activeStudents.length > 0 && (
            <div className="border-t bg-gradient-to-r from-slate-50 via-blue-50 to-indigo-50 px-4 py-3.5 flex items-center justify-between flex-wrap gap-4">
              {/* Left: Summary information */}
              <div className="flex items-center gap-3 flex-wrap">
                <div className="flex items-center gap-2 text-sm">
                  <span className="font-bold text-gray-700">⚡ Attendance Actions:</span>
                  {eligibleUnmarkedStudents.length === 0 ? (
                    <span className="px-2.5 py-1 bg-green-100 text-green-700 rounded-full font-semibold text-xs flex items-center gap-1">
                      <span>🎉</span> All {activeStudents.length} students marked for today
                    </span>
                  ) : (
                    <div className="flex items-center gap-2 text-xs flex-wrap">
                      <span className="px-2.5 py-1 bg-blue-100 text-blue-700 rounded-full font-medium">
                        Marked: <strong>{todayAttendanceCount}/{activeStudents.length}</strong>
                      </span>
                      {todayAbsentOrODCount > 0 && (
                        <span className="px-2.5 py-1 bg-amber-100 text-amber-800 rounded-full font-medium">
                          {todayAbsentOrODCount} Absent/OD
                        </span>
                      )}
                      <span className="px-2.5 py-1 bg-gray-200 text-gray-700 rounded-full font-medium">
                        {eligibleUnmarkedStudents.length} Unmarked
                      </span>
                    </div>
                  )}
                </div>
              ...
              {/* Right: The two buttons */}
              <div className="flex items-center gap-3 flex-wrap">
                {/* 1. Mark Present for All button - ALWAYS shown */}
                <button
                  onClick={handleMarkPresentForAll}
                  disabled={isBulkMarking || eligibleUnmarkedStudents.length === 0}
                  title={eligibleUnmarkedStudents.length === 0 ? 'All eligible students already marked today' : `Directly mark all ${eligibleUnmarkedStudents.length} unmarked student(s) as Present`}
                  className={`px-4 py-2.5 rounded-xl font-semibold text-sm shadow-sm flex items-center gap-2 transition-all ${
                    eligibleUnmarkedStudents.length === 0
                ...
                >
                  {isBulkMarking ? (
                    <><div className="animate-spin w-4 h-4 border-2 border-white border-t-transparent rounded-full"></div> Marking...</>
                  ) : (
                    <>
                      <span>✅</span>
                      <span>Mark Present for All</span>
                      {eligibleUnmarkedStudents.length > 0 && (
                        <span className="ml-1 px-2 py-0.5 bg-emerald-700 text-white rounded-full text-xs font-bold">
                          {eligibleUnmarkedStudents.length}
                        </span>
                      )}
                    </>
                  )}
                ...
                {/* 2. Mark Present for Others button - ONLY ENABLED when teacher marks absent/od for at least 1 student */}
                <button
                  onClick={handleMarkPresentForOthers}
                  disabled={isBulkMarking || !canMarkPresentForOthers}
                  title={
                    !hasAbsentOrODToday
                      ? 'Mark at least one student Absent or OD to enable this button'
                      : eligibleUnmarkedStudents.length === 0
                        ? 'No remaining unmarked students'
                        : `Mark remaining ${eligibleUnmarkedStudents.length} student(s) as Present`
                  }
                  className={`px-4 py-2.5 rounded-xl font-semibold text-sm shadow-sm flex items-center gap-2 transition-all ${
                    canMarkPresentForOthers && !isBulkMarking
                ...
                >
                  {isBulkMarking ? (
                    ...
                    <>
                      <span>👥</span>
                      <span>Mark Present for Others</span>
                      {hasAbsentOrODToday && eligibleUnmarkedStudents.length > 0 && (
                        <span className="ml-1 px-2 py-0.5 bg-blue-700 text-white rounded-full text-xs font-bold">
                          {eligibleUnmarkedStudents.length}
                        </span>
                      )}
                    </>
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-sm w-full">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className={`flex-1 py-3 rounded-xl font-semibold text-white transition-all ${passwordInput ? 'bg-blue-600 hover:bg-blue-700 shadow-md' : 'bg-gray-300 cursor-not-allowed'
                  }`}
              >
                Unlock 🔓
              </button>
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-md w-full">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-gray-500 text-xs">Add to {className} for attendance</p>
                </div>
              </div>
              <button
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  value={newRegNumber}
                  onChange={(e) => { setNewRegNumber(e.target.value); setAddError(''); }}
                  className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:outline-none uppercase font-mono"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  value={newPhone}
                  onChange={(e) => { setNewPhone(e.target.value.replace(/\D/g, '').slice(0, 10)); setAddError(''); }}
                  className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:outline-none font-mono"
                />
                <p className="text-xs text-gray-400 mt-1">10-digit mobile number for SMS alerts</p>
              </div>

              <div className="p-3 bg-blue-50 rounded-xl border border-blue-100 text-xs text-blue-700">
                ℹ️ The new student will appear immediately in today's attendance table. Their attendance record will be saved to Supabase upon marking.
              </div>
            </div>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button
                ...
                className="flex-1 py-3 rounded-xl font-semibold text-white bg-blue-600 hover:bg-blue-700 shadow-md transition-all flex items-center justify-center gap-2"
              >
      ...
      {showHolidayModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-md w-full">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold text-gray-800">🏖️ Mark as Holiday</h2>
              <button
                onClick={() => { setShowHolidayModal(false); setHolidayReason(''); setHolidayError(''); }}
            ...
            <p className="text-sm text-gray-600 mb-4">
              Mark {formatDate(currentDate)} as a holiday. This date won't count as an absent day.
            </p>
            <div className="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g. Public Holiday / College Festival"
                  value={holidayReason}
                  onChange={(e) => { setHolidayReason(e.target.value); setHolidayError(''); }}
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button
                onClick={() => {
                  onMarkHoliday(currentDate, holidayReason.trim() || 'Holiday');
                  setShowHolidayModal(false);
                  setHolidayReason('');
                  setHolidayError('');
                }}
                className="flex-1 py-3 rounded-xl font-semibold text-white bg-orange-500 hover:bg-orange-600 shadow-md transition-all"
              >
                Confirm Holiday
              </button>
            </div>
