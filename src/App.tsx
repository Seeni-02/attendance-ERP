...
const App: React.FC = () => {
  ...
  const handleMarkAttendance = async (studentId: string, status: 'present' | 'absent' | 'od', smsStatus?: 'delivered' | 'failed') => {
    ...
    setAppData(prev => {
      ...
      const newMarkedDates = prevClassData.attendanceMarkedDates.includes(date) ? prevClassData.attendanceMarkedDates : [...prevClassData.attendanceMarkedDates, date];
      return { ...prev, classes: { ...prev.classes, [currentClass]: { ...prevClassData, students: updatedStudents, attendanceRecords: [...prevClassData.attendanceRecords, record], attendanceMarkedDates: newMarkedDates } } };
    });
    ...
    try {
      ...
      if (student) {
        ...
        u.Zone = calculateZone(u.Total_Days_Absent, u.On_Duty_Days);
        await markAttendanceInDB(currentClass, record, u);
        const newMarkedDates = currentClassData.attendanceMarkedDates.includes(date) ? currentClassData.attendanceMarkedDates : [...currentClassData.attendanceMarkedDates, date];
        await updateClassMetaInDB(currentClass, { attendanceMarkedDates: newMarkedDates });
      }
