...
export const exportToCSV = (students: Student[], filename: string, totalWorkingDays: number = 0): void => {
  ...
  const rows = students.map(s => {
    ...
    const totalDays = totalWorkingDays > 0 ? totalWorkingDays : totalDaysRecorded;
...
export const downloadFullStudentDataExcel = (
  ...
  studentData.push([`Total Students: ${activeStudents.length} | Total Working Days: ${classData.totalWorkingDays} | Days Recorded: ${dates.length}`]);
  studentData.push([]);
...
export const downloadMonthWiseExcel = (
  ...
  studentData.push([`Generated: ${formatDate(today)}`]);
  studentData.push([`Total Students: ${activeStudents.length} | Days in Period: ${filteredDates.length}`]);
  studentData.push([]);
...
export const downloadFullBackupExcel = (appData: AppData): void => {
  ...
  summaryData.push([
    ...
    'Working Days', 'Days Recorded', 'Total Records', 'Status', 'Created'
  ]);
  ...
  appData.savedClassNames.forEach(className => {
    ...
    sheetData.push([`Students: ${activeStudents.length} | Working Days: ${classData.totalWorkingDays} | Days Recorded: ${dates.length}`]);
    sheetData.push([]);

    // Student data
    sheetData.push([
      'S.No', 'Register Number', 'Student Name', 'Class', 'Parent Phone',
      'Days Present', 'Days Absent', 'OD Days', 'Total Days', 'Attendance %', 'Zone', 'Status'
    ]);
    ...
    const colWidths = [
      ...
      { wch: 14 }, { wch: 14 }, { wch: 10 }, { wch: 12 }, { wch: 14 }, { wch: 12 }, { wch: 12 }
    ];
