...
export const saveClassToDB = async (className: string, classData: ClassData): Promise<void> => {
  try {
    ...
    if (classData.holidayDates && classData.holidayDates.length > 0) {
      const holidaysToUpsert = classData.holidayDates.map(h => ({
        class_name: className,
        date: h.date,
        reason: h.reason,
      }));
      const { error: holidayError } = await supabase.from('holidays').upsert(holidaysToUpsert, { onConflict: 'class_name,date' });
...
// ==================== REMOVE STUDENT ====================
export const removeStudentFromDB = async (className: string, studentId: string): Promise<void> => {
  try {
...
export const removeHolidayFromDB = async (className: string, date: string): Promise<void> => {
  try {
    const { error } = await supabase.from('holidays').delete().match({ class_name: className, date: date });
    if (error) throw error;
  } catch (error) {
